...
const AnalyzeSubagentTelemetryParams = Type.Object({
	...
	focus: Type.Optional(StringEnum(["reliability", "cost", "ux", "all"] as const, { description: "Telemetry area to analyze. Defaults to all." })),
...
export function registerAnalyzeSubagentTelemetryTool(pi: ExtensionAPI, getConfig: () => SubagentTelemetryConfig): void {
	pi.registerTool({
		...
		promptGuidelines: [
			"Use analyze_subagent_telemetry to inspect worker reliability, cost, or UX before proposing subagent architecture/configuration changes.",
