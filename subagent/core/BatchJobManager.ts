...
import type { AgentRecord, RoutingDecision } from "./AgentTypes.ts";
import type {
	BatchInputRow,
	BatchJob,
	BatchJobCounts,
...
export interface BatchJobManagerOptions {
	...
	forkRoutingDecision?: (decision: RoutingDecision, item: BatchJobItem) => Promise<RoutingDecision>;
}
...
export class BatchJobManager {
	...
	private readonly forkRoutingDecision?: (decision: RoutingDecision, item: BatchJobItem) => Promise<RoutingDecision>;
	...
	constructor(options: BatchJobManagerOptions) {
		...
		this.forkRoutingDecision = options.forkRoutingDecision;
	...
	private async startItem(job: BatchJob, item: BatchJobItem): Promise<void> {
		...
		try {
			const workerRoutingDecision = job.routingDecision && this.forkRoutingDecision
				? await this.forkRoutingDecision(job.routingDecision, item)
				: job.routingDecision;
