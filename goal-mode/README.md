...
- **`/goal <task>`** — Puts the agent into autonomous execution mode.  It will
  make reasonable assumptions when information is missing, avoid asking
  open-ended questions, work step-by-step, and report progress as it goes.
- **`/no-goal`** — Exits goal mode and restores normal collaboration.
- **`/goal-status`** — Shows the current goal, turn count, and extracted
  progress checklist.
- **`Ctrl+Alt+G`** — Shortcut to exit goal mode.

## How it works

1. When `/goal` is invoked, the extension stores the task description and
   activates goal mode.
