...
function metricValue(query: string): number {
	if (query.includes("_count")) return 20;
	if (query.includes("messages_total")) return query.includes("outcome=\"succeeded\"") ? 18 : 20;
	if (query.includes("first_progress") && query.includes("bucket")) return 12;
	if (query.includes("queue_duration") && query.includes("bucket")) return 6;
	if (query.includes("agent_duration") && query.includes("bucket")) return 20;
	if (query.includes("timeout|lost")) return 0.4;
	if (query.includes("rpc_requests")) return query.includes("failed") ? 0.2 : 20;
	if (query.includes("context_recovery")) return query.includes("outcome=\"succeeded\"") ? 16 : 20;
	if (query.includes("cost_USD")) return 2;
	if (query.includes("tokens_total")) return 1_000;
	if (query.includes("observations_total")) return query.includes("outcome=\"succeeded\"") ? 18 : 20;
	if (query.includes("outcome=\"succeeded\"")) return 18;
	return 20;
}
...
test("analysis client reports zero-volume telemetry as insufficient rather than a critical failure", async () => {
	await withServer((request, response) => {
		const url = new URL(request.url ?? "/", "http://localhost");
		response.setHeader("content-type", "application/json");
		if (url.pathname === "/api/v1/query") {
			response.end(JSON.stringify({ status: "success", data: { resultType: "vector", result: [{ metric: {}, value: [Date.now() / 1_000, "0"] }] } }));
			return;
		}
		response.end(JSON.stringify({ data: [] }));
	}, async (origin) => {
		const config = loadSubagentTelemetryConfig({ env: { PI_SUBAGENT_PROMETHEUS_URL: origin, PI_SUBAGENT_JAEGER_URL: origin } });
		const snapshot = await new TelemetryAnalysisClient(config).analyze({ focus: "reliability", comparePreviousPeriod: false, maxTraceExamples: 0 });
		assert.equal(snapshot.violations.length, 0);
		assert(snapshot.warnings.some((warning) => warning.includes("Insufficient Subagent samples")));
		assert(snapshot.metrics.filter((metric) => metric.unit === "ratio").every((metric) => !metric.available && metric.error === "zero_denominator"));
	});
});
